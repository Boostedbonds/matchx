...
// ── 15 predefined avatars stored in /public/avatars/ ──────────────────────
...
const STYLES = `
  ...
  .ps-bar-fill {
    height: 100%; transition: width 0.8s cubic-bezier(0.25,1,0.5,1);
    position: relative;
  }
  .ps-bar-fill::after {
  ...
  .match-row {
    ...
    padding: 14px 0; transition: background 0.15s; cursor: default;
  }
  .match-result-dot {
  ...
  .avatar-preset {
    ...
    transition: all 0.2s; background: #1a1f2e;
  }
  .avatar-preset:hover { border-color: rgba(0,255,200,0.5); transform: scale(1.05); }
  .avatar-preset.selected { border-color: #00ffc8; box-shadow: 0 0 16px rgba(0,255,200,0.3); }
  /* Fallback colored presets */
  .avatar-preset-fb {
...
// Emoji fallback avatars for when image files aren't set up yet
const EMOJI_AVATARS = ["🦅","🐯","🦁","🐉","🦊","🐺","🦈","🐻","🦋","🦅","🎯","⚡","🔥","💎","🏆"];
...
function computePlaystyle(p) {
  ...
  // Heuristic formulas — can be replaced with real data
  const winRate = total > 0 ? (p.wins / total) : 0.5;
  const attack   = Math.min(100, Math.round(50 + winRate * 40 + (elo - 1500) / 30));
  const defense  = Math.min(100, Math.round(40 + (1 - winRate) * 30 + total * 0.5));
  const stamina  = Math.min(100, Math.round(30 + total * 1.2));
  const technique= Math.min(100, Math.round(40 + (elo - 1500) / 20));
  const experience = Math.min(100, Math.round(total * 3));

  return { attack, defense, stamina, technique, experience };
}
...
export default function Profile({ user, onNav, onLogout }) {
  ...
  async function fetchPlayer() {
    const uid = user?.id || localStorage.getItem("player_id");
    let query = uid
      ? supabase.from("players").select("*").eq("auth_id", uid).maybeSingle()
      : supabase.from("players").select("*").eq("id", localStorage.getItem("player_id")).maybeSingle();
    ...
    if (!data && user?.id) {
      const { data: np } = await supabase.from("players").insert({
        ...
        avatar_url: getRandomEmojiAvatar(),
      }).select().single();
      data = np;
    }
    setPlayerData(data);
    if (data) fetchMatchHistory(data.id);
  }
        ...
        <div className="prof-inner">
          ...
          <div className="prof-hero">
            <div className="prof-avatar-wrap">
              {playerData.avatar_url && !playerData.avatar_url.startsWith("/avatars/")
                ? <img src={playerData.avatar_url} className="prof-avatar" alt="avatar" />
                : <div className="prof-avatar-fallback">
                    {playerData.avatar_url?.startsWith("/avatars/")
                      ? <img src={playerData.avatar_url} style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} alt="" />
          ...
          <div className="badges-grid" style={{ marginBottom: 32 }}>
            {BADGES.map(b => {
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="modal-box">
            ...
            <div className="modal-divider">
              ...
              <div className="modal-divider-text">Choose a preset</div>
              <div className="modal-divider-line" />
            </div>
            ...
            <div className="modal-divider">
              <div className="modal-divider-line" />
              <div className="modal-divider-text">Or use your own photo</div>
              <div className="modal-divider-line" />
            </div>
            ...
            <div className="camera-section">
              ...
              {showCamera && !capturedImage && (
                <>
                  ...
                  <button className="modal-confirm-btn" onClick={capturePhoto}>
                    ● Capture Photo
                  </button>
                </>
              )}
...
function getRandomEmojiAvatar() {
  return `emoji:${EMOJI_AVATARS[Math.floor(Math.random() * EMOJI_AVATARS.length)]}`;
}