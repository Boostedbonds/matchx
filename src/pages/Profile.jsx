...
function Profile({ user, onNav, onLogout }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080a0f", color: "#fff", display: "flex" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          ...
          .pqs-val { font-size: 24px !important; }
          .tabs {
      ...
      {/* ── Sidebar — passed directly, no wrapper div ── */}
      ...
      <div className="main">
        ...
        <div className="profile-hero">
          <div className="profile-header">
            <div className="user-avatar-lg">{user?.init || user?.name?.slice(0,2).toUpperCase()}</div>
          ...
          {activeTab === "Overview" && (
            <div className="section-grid">
              <div className="card">
                ...
                {[
                  { label: "Matches Played", val: (user?.wins || 0) + (user?.losses || 0), cls: "" },
                  { label: "Wins",           val: user?.wins,   cls: "green" },
                  { label: "Losses",         val: user?.losses, cls: "red" },
                  { label: "Win Streak",     val: `${user?.streak || 0} 🔥`, cls: "gold" },
                ...
                {[
                  ...
                  <div key={i} style={{ marginBottom: 20 }}>
                    ...
                    <div className="progress-bar">
                      <div className="progress-fill" style={{ width: `${Math.min(100, ((s.val || 0) / s.max) * 100)}%`, background: s.color }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              {[
                ...
                <div className="history-item" key={i}>
                  ...
                  <div className="result-tag" style={{
                    background: m.result === "win" ? "rgba(0,255,100,0.08)" : "rgba(255,50,80,0.08)",
                    border: `1px solid ${m.result === "win" ? "rgba(0,255,100,0.2)" : "rgba(255,50,80,0.2)"}`,
                    color: m.result === "win" ? "#00ff64" : "#ff3250"
                  }}>
                    {m.result}
                  </div>
                </div>
            ...
            <div className="card">
              ...
              <div className="badge-grid">
                {[
                  ...
                  { icon: "🎯", name: "Sharpshooter", desc: "Win 25 matches",        earned: (user?.wins || 0) >= 25 },
                  { icon: "🛡️", name: "Iron Wall",    desc: "2,500 points earned",   earned: (user?.points || 0) >= 2500 },
                  { icon: "👑", name: "Top 10",       desc: "Reached global rank #10", earned: (user?.rank || 999) <= 10 },
                  { icon: "🌟", name: "Rising Star",  desc: "Earn 5,000 points",     earned: (user?.points || 0) >= 5000 },
