...
const styles = `
  ...
  .db-welcome { margin-top: 12px; font-size: 18px; font-weight: 500; color: rgba(232,224,208,0.6); letter-spacing: 0.05em; }
  ...
  .db-stat-label { font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 0.18em; color: rgba(212,175,55,0.6); text-transform: uppercase; margin-bottom: 6px; }
  .db-stat-value { font-family: 'Bebas Neue', sans-serif; font-size: 36px; letter-spacing: 0.05em; color: #ffd700; line-height: 1; }
  ...
  .db-section-title .live-dot {
    ...
    box-shadow: 0 0 12px #00e6a0; animation: pulse 1.4s ease-in-out infinite;
  }

  @keyframes pulse { 0%, 100% { opacity:1; transform:scale(1); } 50% { opacity:0.5; transform:scale(0.7); } }

  .db-count-badge { font-family: 'JetBrains Mono', monospace; font-size: 11px; letter-spacing: 0.15em; color: rgba(212,175,55,0.5); padding: 4px 10px; border: 1px solid rgba(212,175,55,0.15); }

  .db-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 16px; }
  ...
  .db-card:hover {
    border-color: rgba(212,175,55,0.35); transform: translateY(-3px);
    box-shadow: 0 12px 40px rgba(0,0,0,0.5), 0 0 0 1px rgba(212,175,55,0.08);
  }

  .db-card:hover::after { opacity: 1; }

  .db-card-top { padding: 16px 20px; border-bottom: 1px solid rgba(212,175,55,0.08); display: flex; align-items: center; justify-content: space-between; }
  ...
  .db-avatar {
    ...
    font-family: 'Bebas Neue', sans-serif; font-size: 14px; letter-spacing: 0.05em; color: #ffd700;
  }

  .db-vs { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: rgba(212,175,55,0.4); letter-spacing: 0.1em; }
  ...
  .db-match-name { font-size: 15px; font-weight: 700; letter-spacing: 0.04em; color: #e8e0d0; margin: 0 0 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

  .db-match-meta { display: flex; gap: 12px; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: rgba(232,224,208,0.35); letter-spacing: 0.08em; }

  .db-card-foot { padding: 12px 20px; border-top: 1px solid rgba(212,175,55,0.06); }
  ...
  .db-empty h2 { font-family: 'Bebas Neue', sans-serif; font-size: 36px; letter-spacing: 0.1em; color: rgba(212,175,55,0.5); margin: 0 0 8px; }
  .db-empty p  { color: rgba(232,224,208,0.35); font-size: 15px; margin: 0 0 32px; letter-spacing: 0.05em; }
  ...
  .db-create-btn {
    ...
    font-size: 18px; letter-spacing: 0.1em; border: none; cursor: pointer;
    clip-path: polygon(8px 0%, 100% 0%, calc(100% - 8px) 100%, 0% 100%);
    transition: all 0.2s;
  }
  ...
  .db-loading {
    display: flex; align-items: center; justify-content: center; gap: 12px; padding: 80px;
    color: rgba(212,175,55,0.5); font-family: 'JetBrains Mono', monospace; font-size: 12px; letter-spacing: 0.15em;
  }

  .db-spinner { width: 20px; height: 20px; border: 2px solid rgba(212,175,55,0.15); border-top-color: #d4af37; border-radius: 50%; animation: spin 0.8s linear infinite; }
...
function MatchCard({ match, onNav }) {
  const badge = getBadge(match.status);
  return (
    <div className="db-card" onClick={() => onNav("scorer")}>
...
export default function Dashboard({ onNav, onLogout, user }) {
  ...
  // Still pull displayName from AuthContext if onNav/user not passed
  const { user: authUser, player } = useAuth();
  const displayName =
    user?.name || player?.name || authUser?.email?.split("@")[0] || "Player";

  useEffect(() => {
    loadDashboardData();

    const subscription = subscribeToLiveMatches((matches) => {
      setStats((prev) => ({ ...prev, liveMatches: matches, liveNow: matches.length }));
    });
      ...
      <div className="db-root">
        <div className="db-inner">
          ...
          <header className="db-header">
            ...
            <button className="db-new-match-btn" onClick={() => onNav("setup")}>
              ⚡ NEW MATCH
            </button>
          </header>
          ...
          {/* LOADING */}
          {loading && (
            <div className="db-loading"><div className="db-spinner" />LOADING MATCHES...</div>
          )}
            ...
            <section className="db-section">
              ...
              <div className="db-grid">
                {stats.liveMatches.map((m, i) => (
                  <MatchCard key={m.id || i} match={m} onNav={onNav} />
                ))}
              </div>
            </section>
          )}
          ...
          {!loading && stats.liveMatches.length === 0 && stats.recentMatches.length === 0 && (
            <div className="db-empty">
              ...
              <button className="db-create-btn" onClick={() => onNav("setup")}>
                ⚡ CREATE NEW MATCH
              </button>
            </div>
          )}
