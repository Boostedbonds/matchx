...
const styles = `
  ...
  .db-match-name { font-size: 14px; font-weight: 700; letter-spacing: 0.04em; color: #e8e0d0; margin: 0 0 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .db-match-meta { display: flex; gap: 12px; font-family: 'JetBrains Mono', monospace; font-size: 9px; color: rgba(232,224,208,0.35); letter-spacing: 0.08em; }
  ...
  .db-spinner { width: 18px; height: 18px; border: 2px solid rgba(212,175,55,0.15); border-top-color: #d4af37; border-radius: 50%; animation: spin 0.8s linear infinite; }
...
export default function Dashboard({ onNav, onWatchMatch }) {
  ...
  useEffect(() => {
    ...
    // Realtime: refresh live matches on any match change
        ...
        <div className="db-inner">
          ...
          {!loading && stats.liveMatches.length === 0 && stats.recentMatches.length === 0 && (
            <div className="db-empty">
              ...
              <button className="db-create-btn" onClick={() => onNav("setup")}>⚡ CREATE NEW MATCH</button>
            </div>
          )}
