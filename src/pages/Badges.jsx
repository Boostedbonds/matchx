    ...
    <div style={{ minHeight: "100vh", background: "#080a0f", color: "#fff", display: "flex" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .tier-summary {
          padding: 18px 20px; background: #0d0f15;
        ...
        .tier-summary::before {
          ...
          width: 3px; height: 100%; background: var(--tc);
        }
        ...
        .badge-card {
          padding: 20px 16px; text-align: center; border: 1px solid;
          position: relative; overflow: hidden; transition: all 0.3s;
        }

        .badge-card.locked   { filter: grayscale(1); opacity: 0.35; }
        .badge-card.unlocked:hover { transform: translateY(-3px); box-shadow: 0 8px 30px rgba(0,0,0,0.3); }
            ...
            return (
              <div className="tier-summary" key={tier} style={{ "--tc": tc.color }}>
                ...
                <div className="tier-count">
                  {done.length}
                  <span style={{ fontSize: 18, color: "rgba(255,255,255,0.2)" }}>/{total.length}</span>
                </div>
          ...
          return (
            <div className="tier-section" key={tier}>
              <div className="tier-header">
                ...
                <div style={{ fontFamily: "'Rajdhani'", fontSize: 12, color: tc.color, letterSpacing: 1 }}>
                  {done.length}/{tierBadges.length}
                </div>
              </div>

              <div className="badges-grid">
                {tierBadges.map((b, i) => {
                  const earned = isUnlocked(b, user);
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderColor:  earned ? tc.border : "rgba(255,255,255,0.06)",
                      }}
                    >
                      {earned && <div className="unlocked-stamp">✓ Unlocked</div>}
                      <div className="badge-card-icon">{b.icon}</div>
                      <div className="badge-card-name" style={{ color: earned ? tc.color : "rgba(255,255,255,0.4)" }}>{b.name}</div>
